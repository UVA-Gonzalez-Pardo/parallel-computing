...
int calcular_max(int * mapa, int rows, int cols, Registro * registros){
	...
	#pragma omp parallel for firstprivate(max)
		...
		for(int i=0; i<rows; i++)
		{
			for(int j=0; j<cols; j++)
			{
				//printf("Valor a max: %d maximo es: %d hilo: %d \n",m(i,j),max,omp_get_thread_num());
				if(m(i,j)>max)
				{
					max = m(i,j);
					...
					registros[omp_get_thread_num()].valor=max;

					//printf("i,j, valor guardados:%d, %d, %d hilo: %d\n",i,j,max,omp_get_thread_num());	
	...
	{
		//printf("registro cont: %d   ",registros[k]);
		if(registros[k].valor>max)
		{
			max=registros[k].valor;
			registros[NUM_HILOS].valor=max;
			registros[NUM_HILOS].x=registros[k].x;
			registros[NUM_HILOS].y=registros[k].y;
