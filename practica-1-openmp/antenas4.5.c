 ...
 */
Antena nueva_antena(int x, int y)
{	
	Antena antena = {x,y};
	return antena;
}
...
int main(int nargs, char ** vargs)
{
	...
	while(1)
	{
		...
		if (calcular_max(mapa, rows, cols, registros,num_hilos) <= distMax) break;	
		
		// Incrementamos el contador
		nuevas++;
		
		// Calculo de la nueva antena y actualización del mapa
		//Antena antena = nueva_antena( registros[num_hilos].x,registros[num_hilos].y);
		actualizar(mapa, rows, cols, nueva_antena( registros[num_hilos].x,registros[num_hilos].y));
	}
