...
void actualizar(int ** mapa, int rows, int cols, Antena * antena)
{
	...
	for(int r = 1; r < (rows + cols); r++)
	{
		...
		// Si arriba.y esta fuera del tablero, iniciamos delta en la primera
		// interseccion del rombo con el tablero por arriba.
		int delta;
		if (arriba.y < 0){
			delta = abs(r - miAntena.y);
			//printf("NEW:%d\n",delta);
		} else {
			delta = 0;
		}
		...
		for( ; delta <= r; delta++)
		{
			//printf("deltaArriba: %d\n", delta);
			// Arriba - Descendiendo por la Derecha
			if( (arriba.y + delta >= 0) && (arriba.x + delta < cols) &&
				(mapa[arriba.y + delta][arriba.x + delta] > distancia) )
			{
				mapa[arriba.y + delta][arriba.x + delta] = distancia;
				flag = 1;
				//printf("Arriba derecha: (%d, %d)\n", arriba.y + delta, arriba.x + delta);
			}
			// Arriba - Descendiendo por la Izquierda
			if( (arriba.y + delta >= 0) && (arriba.x - delta >= 0) &&
				(mapa[arriba.y + delta][arriba.x - delta] > distancia) )
			{
				mapa[arriba.y + delta][arriba.x - delta] = distancia;
				flag = 1;
				//printf("Arriba izquierda: (%d, %d)\n", arriba.y + delta, arriba.x - delta);
			}
		}//delta

		// Si abajo.y esta fuera del tablero, iniciamos delta en la primera
		// interseccion del rombo con el tablero por abajo.
		if (abajo.y >= rows){
			delta = abs(miAntena.y + r - rows);
			//printf("NEW2:%d\n",delta);
		} else {
			delta = 0;
		}
		...
		{
			//printf("deltaAbajo: %d\n", delta);
			// Abajo - Ascendiendo por la Derecha
			if( (abajo.y - delta < rows) && (abajo.x + delta < cols) &&
				(mapa[abajo.y - delta][abajo.x + delta] > distancia) )
			{
				mapa[abajo.y - delta][abajo.x + delta] = distancia;
				flag = 1;
				//printf("Abajo derecha: (%d, %d)\n", abajo.y - delta, abajo.x + delta);
			}
			// Abajo - Ascendiendo por la Izquierda
			if( (abajo.y - delta < rows) && (abajo.x - delta >= 0) &&
				(mapa[abajo.y - delta][abajo.x - delta] > distancia) )
			{
				mapa[abajo.y - delta][abajo.x - delta] = distancia;
				flag = 1;
				//printf("Abajo izquierda: (%d, %d)\n", abajo.y - delta, abajo.x - delta);
			}
		}//delta
